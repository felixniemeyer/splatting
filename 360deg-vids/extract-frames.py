...
class Perspective: 
    ...
    def filename(self): 
        return f"p{self.pitch}y{self.yaw}"
...
circles = [
    Circle(10, args.fov * 0.5, 90),
    Circle(55, args.fov * 0.7, 60),
    Circle(-35, args.fov * 0.5, 200), # nach unten 
#    Circle(90, args.fov * 1.1, 60),
]
...
for i, perspective in enumerate(perspectives): 
    ...
    if args.separate_folders: 
        # make folder
        folder = f"{args.outdir}/{perspective.filename()}"
        mkfolders += f"mkdir -p {folder} \n"
        outputs += f"-map \"[v{i}]\" -r {args.rate} \"{folder}/%05d.jpg\" \\\n"
    else:
